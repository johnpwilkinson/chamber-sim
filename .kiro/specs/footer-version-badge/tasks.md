...
- [x] 2. FooterVersionBadge component
...
- [x] 3. Mount FooterVersionBadge in App
- [x] 3.1 Mount `<FooterVersionBadge />` in `src/App.tsx` immediately alongside the existing `<CommandPalette />` mount, with no other markup changes and no other file added or modified
  _Resolved: vitest.config.ts is a separate Vite config from vite.config.ts and does not inherit vite.config.ts's  define: { __APP_VERSION__ }  entry, so under Vitest the bare identifier __APP_VERSION__ is undeclared. Rather than editing vitest.config.ts (out of the closed file list in 11.3), footer-version-badge.tsx now reads the version via `typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : 'unknown'`, which is a safe existence guard (not import.meta.env.DEV/PROD gating) that renders identical markup in dev/prod builds where the define is present, and falls back gracefully under Vitest where it is not. App.tsx now mounts <FooterVersionBadge /> alongside <CommandPalette />; all 72 tests, including the App mount-point integration tests, pass._
