...
|---|---|---|
| `docs/drills/drill-grok-hands.md` | Owns | The drill marker document; the sole artifact of this feature. |

## Boundary Commitments

| Commitment | Detail |
|---|---|
| Drill isolation | `src/drill/drill-grok-hands` MUST NOT import `src/components` — the drill owns no source modules; this guards the reserved drill source path. |
| Product isolation | `src/components` MUST NOT import `src/drill/drill-grok-hands` — product code never depends on a throwaway drill marker. |
...
- **Docs-only implementation** — the implementer creates exactly one file,
  `docs/drills/drill-grok-hands.md`, and does not modify `package.json` or any
  source file. The spec's own process artifacts (`.kiro/specs/drill-grok-hands/`,
...
- **Inert boundary rules by construction** — the two table commitments
  ...
  `src/drill/drill-grok-hands`, which this drill never creates. They are meant to
...
- **Throwaway grade** — the marker exists to prove the runway; deleting it
  (and the `sdd-drill-grok-hands-*` rules) after the drill PR merges is
  sanctioned.

## Concrete Shape

`docs/drills/drill-grok-hands.md` renders as a heading line `# drill-grok-hands` followed
